...
KAC=[cl,ki,kk,kc,kb,kd,ke,kh,kg,sw,sx,sy]
...
#------------------------Auto Join-------------------------------
	...
	if op.type == 13:
        ...
        if op.type == 13:
            ...
            if op.param3 in Imid:
                if op.param2 in mid:
                    X = sx.getGroup(op.param1)
                    X.preventJoinByTicket = False
                    sx.updateGroup(X)
                    Ti = sx.reissueGroupTicket(op.param1)
                    cl.acceptGroupInvitationByTicket(op.param1,Ti)
                    X.preventJoinByTicket = True
                    cl.updateGroup(X)
                    Ti = cl.reissueGroupTicket(op.param1)
            if op.param3 in Imid:
                if op.param2 in mid:
                    X = sy.getGroup(op.param1)
                    X.preventJoinByTicket = False
                    sy.updateGroup(X)
                    Ti = sy.reissueGroupTicket(op.param1)
                    cl.acceptGroupInvitationByTicket(op.param1,Ti)
                    X.preventJoinByTicket = True
                    cl.updateGroup(X)
                    Ti = cl.reissueGroupTicket(op.param1)
...
#---------------------------------------------------------------------------------------------
		...
		if wait["Protectgroupname"] == False:
			...
			cl.sendText(msg.to,"Gnamelock already off")
#---------------------------------------------------------------------------------------------
	    elif msg.text in ["Siri on","Nikung on","Sw:on"]:
	      if msg.from_ in admin:
                ...
                if wait["cancelprotect"] == True:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"On")
#-----------------------------------------------------------------------------------------
            elif msg.text in ["Siri off","Nikung off","Sw:off"]:
              if msg.from_ in admin:
...
#----------------------------------------------------------------------------------------------
		...
		if wait["goodbye"] == False:
	      ...
	      if msg.from_ in admin:
            ...
            elif msg.text in ["Bang","Bg"]:
            	msg.contentType = 13
                msg.contentMetadata = {'mid': 'u0fff0333558535e9cdbe1df7fad7e72b'}
                msg.contentMetadata = {'mid': 'uf50d888821632d32461e37153ac775c0'}
                msg.contentMetadata = {'mid': 'u479ac059fe16c72a87bee2d0901e8c3f'}
                cl.sendMessage(msg)
